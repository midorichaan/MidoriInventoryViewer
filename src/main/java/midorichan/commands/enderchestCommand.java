    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("enderchest")) {
            if (!(sender instanceof Player)) {
                ...
                if (!(p.hasPermission("midoriinventory.enderchest"))) {
                ...
                if (args.length == 0) {
                    ...
                    p.openInventory(i);
                    p.sendMessage(plugin.getPrefix() + p.getName() + "のエンダーチェストを開きました");
                    ...
                    p.openInventory(i);
                    p.sendMessage(plugin.getPrefix() + target.getName() + "のエンダーチェストを開きました");
                } else {
